...
setData(props.options);
  ...
  //For showing data in dropdown based on input plus all 
  const options = data.filter((item) => item.toLowerCase().includes(search.trim().toLowerCase())).map((item) => (
    <Combobox.Option value={item} key={item} active={value.includes(item)}>
      <Group gap="sm">
        <Checkbox size='xs' color='bright-sun.4' checked={value.includes(item)}
          onChange={() => {}}
          aria-hidden
          tabIndex={-1}
  ...
  return (
    <Combobox store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
      <Combobox.DropdownTarget>
        <PillsInput onClick={() => combobox.toggleDropdown()}  variant='unstyled' rightSection={<IconSelector />}
        leftSection={
